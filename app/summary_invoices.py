...
# ===== Tab3: 1) รายงานตามช่วงเวลา (แยก/ไม่แยกบริษัท) =====
@router.get("/api/invoices/report/periods")
def api_invoice_report_periods(
    granularity: str = Query("day", pattern="^(day|month|year)$"),
    ...
    # เงื่อนไข join (รองรับ invoice_number หรือ idx เป็น string)
    join_cond = or_(
        itm.invoice_number == inv.invoice_number,
        itm.invoice_number == cast(inv.idx, String),
    )
    ...
    q = db.query(*cols, func.count(func.distinct(inv.idx)).label("count"),
                 func.coalesce(sum_amount, 0).label("amount")).outerjoin(itm, join_cond)
    ...
    # group by
    if split_by_company:
        q = q.group_by("period", "company").order_by("period", "company")
    else:
        q = q.group_by("period").order_by("period")
    ...
    for row in q.all():
        # แยกค่า
        if split_by_company:
            period, company, cnt, amount = row
        else:
            period, cnt, amount = row
            company = None
...
# ===== NEW: 2) รายงาน "แบบสรุป" (รวมทั้งช่วง / แยก/ไม่แยกบริษัท) =====
@router.get("/api/invoices/report/overall")
def api_invoice_report_overall(
    granularity: str = Query("day", pattern="^(day|month|year)$"),
    split_by_company: bool = Query(False),
    start: Optional[str] = Query(None, description="YYYY-MM-DD"),
    end: Optional[str] = Query(None, description="YYYY-MM-DD"),
    ...
    """
    รวมยอดทั้งช่วง (ไม่แตก period) — ถ้า split_by_company=True จะรวมต่อบริษัท
    """
    inv = models.Invoice
    itm = models.InvoiceItem
    ...
    # กรองตามช่วงเดียวกับแบบ periods
    if granularity == "day":
        d1 = _to_date(start); d2 = _to_date(end)
        if d1: q = q.filter(inv.invoice_date >= d1)
        if d2: q = q.filter(inv.invoice_date <= d2)
    ...
    if split_by_company:
        q = q.group_by("company").order_by("company")

    rows = q.all()
    out = []
    for row in rows:
        if split_by_company:
            company, cnt, amount = row
        else:
            company, (cnt, amount) = None, row
        amount = _money(amount)
        discount = 0.0
        before_vat = amount - discount
        vat = before_vat * VAT_RATE
