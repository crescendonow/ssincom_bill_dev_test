...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const customerDisplayName = (prename, fname) => {
        const prefix = (prename || '').trim();
        const name = (fname || '').trim();
        if (!prefix || !name || name.startsWith(prefix)) return name || prefix;
        return `${prefix} ${name}`;
    };
    const createThaiDatePicker = (input, defaultDate) => {
    ...
    const dateValue = (picker, input) => picker?.getDate?.() || input?.value || '';
    const now = new Date();
    const todayISO = localISODate(now);
    const firstOfMonthISO = localISODate(new Date(now.getFullYear(), now.getMonth(), 1));
    const billDatePicker = createThaiDatePicker(billDateInput, todayISO);
    const startDatePicker = createThaiDatePicker(startDateInput, firstOfMonthISO);
    const endDatePicker = createThaiDatePicker(endDateInput, todayISO);
    const searchStartDatePicker = createThaiDatePicker(document.getElementById('searchStartDate'));
    const searchEndDatePicker = createThaiDatePicker(document.getElementById('searchEndDate'));

    const waitForPrintFonts = () => {
    ...
    const preloadPrintFonts = () => waitForPrintFonts().catch(error => console.warn('Thai print fonts are not ready yet.', error));
    const printBillNote = async () => {
        ...
        try {
            ...
            console.error(error);
            alert('\u0e44\u0e21\u0e48\u0e2a\u0e32\u0e21\u0e32\u0e23\u0e16\u0e42\u0e2b\u0e25\u0e14\u0e41\u0e1a\u0e1a\u0e2d\u0e31\u0e01\u0e29\u0e23\u0e44\u0e17\u0e22\u0e2a\u0e33\u0e2b\u0e23\u0e31\u0e1a\u0e01\u0e32\u0e23\u0e1e\u0e34\u0e21\u0e1e\u0e4c \u0e01\u0e23\u0e38\u0e13\u0e32\u0e25\u0e2d\u0e07\u0e43\u0e2b\u0e21\u0e48\u0e2d\u0e35\u0e01\u0e04\u0e23\u0e31\u0e49\u0e07');
        } finally {
    ...
    async function loadAllCustomers() {
        try {
            ...
            customerList.innerHTML = customersCache.slice(0, 5000).map(c => {
                ...
                const name = customerDisplayName(c.prename, c.fname ?? c.customer_name);
    ...
    function resolveCustomerSelection() {
        ...
        if (found) {
            ...
            const fixedLabel = [(found.personid ?? '').trim(), customerDisplayName(found.prename, found.fname ?? found.customer_name)].join(' | ');
    ...
    async function generateBill() {
        ...
        const start = dateValue(startDatePicker, startDateInput);
        const end = dateValue(endDatePicker, endDateInput);
        ...
        try {
            ...
            const selectedBillDate = dateValue(billDatePicker, billDateInput) || todayISO;
    ...
    async function saveBillNote() {
        ...
        const payload = {
            ...
            bill_date: dateValue(billDatePicker, billDateInput) || todayISO,
    ...
    function renderBillDocument(data) {
        ...
        for (let i = 0; i < totalPages; i++) {
            ...
            const fullName = customerDisplayName(data.customer.prename, data.customer.name);
    ...
    printBtn.addEventListener('click', printBillNote);
    ...
    // --- Initial Load ---
    billDatePicker.setDate(todayISO, false);
    endDatePicker.setDate(todayISO, false);
    startDatePicker.setDate(firstOfMonthISO, false);
